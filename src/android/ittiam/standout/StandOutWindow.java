...
import android.ittiam.standout.StandOutWindow.Window.Editor;
...
public abstract class StandOutWindow extends Service {
	...
	/* Vikas */
	static Context exploreUIContext;
	static View origFrameLayout;
	...
	public static void show(Context context,
		...
		/* Vikas */
		exploreUIContext = context;
		context.startService(getShowIntent(context, cls, id));
	}

	// Added - Vikas
	/**
	 * Reposition and resize an already shown window corresponding to the id
	 * 
	 * @param context
	 *            A Context of the application package implementing this class.
	 * @param cls
	 ...
	 *            to create and manage the window.
	 * @param id
	 *            The id representing this window. 
	 * @param x
	 *            The new x co-ordinate of the window.
	 * @param y
	 *            The new y co-ordinate of the window. 
	 * @param w
	...
	public static void modify(Context context,
			Class<? extends StandOutWindow> cls, int id, int x, int y, int w, int h) {
		Window window = getCache(id, cls);
		Log.d("Standout1", " Changing Position to "+x+" "+y);
		window.edit().setPosition(x, y).commit();
		Log.d("Standout1", " Changing Size to "+w+" "+h);
		window.edit().setSize(w, h).commit();
	}
	 ...
	 * @see #show(int)
	 */
	public static void addBorder(Context context,
			Class<? extends StandOutWindow> cls, int id) {
		//Window window = getCache(id, cls);

		Log.d(TAG, " Setting Border");
		origFrameLayout.setBackgroundResource(R.drawable.border);
	}
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		Log.d("Standout", "Returning from onStartCommand");
	...
	protected Notification getPersistentNotification(int id) {
		...
		/* Vikas -Hack Removing Notification Message */
	...
	protected final synchronized Window show(int id) {
		...
		try {
			...
			Log.d("Standout1", "Adding window to windowManager id = "+id);
				...
				if (animation != null) {
					animation.setAnimationListener(new AnimationListener() {
						...
						public void onAnimationEnd(Animation animation) {
							...
							Log.d("Standout", "Removing window from windowManager 1");
					...
					// remove the window from the window manager
					Log.d("Standout", "Removing window from windowManager 2");
					mWindowManager.removeView(window);
				}
			} catch (Exception ex) {
				ex.printStackTrace();
		...
		try {
			...
			if (animation != null) {
				animation.setAnimationListener(new AnimationListener() {
					...
					public void onAnimationEnd(Animation animation) {
						// remove the window from the window manager
						Log.d("Standout", "Removing window from windowManager 11");
						mWindowManager.removeView(window);
						window.visibility = Window.VISIBILITY_GONE;
				...
				mWindowManager.removeView(window);
				Log.d("Standout", "Removing window from windowManager 22");
				// remove view from internal map
			...
			case MotionEvent.ACTION_UP:
				...
				if (event.getPointerCount() == 1) {
					...
					if (tap
						...
						/* Vikas - Detected Double tap - I want to notify the application with the class that was tapped */
						//StandOutWindow.this.bringToFront(id);
						//((ExploreUIActivity) ExploreUIActivity.context).doubleTapDetected(window.cls);
	...
	public class Window extends FrameLayout {
		...
		public Window(int id) {
			...
			if (Utils.isSet(flags, StandOutFlags.FLAG_DECORATION_SYSTEM)) {
				...
				/* Vikas - Store the frameLayout */
				origFrameLayout = content;
		...
		public class Editor {
			...
			public void commit() {
				if (mParams != null) {
					Log.d("Standout1", " Updating Layout "+mParams.x+" "+mParams.y
